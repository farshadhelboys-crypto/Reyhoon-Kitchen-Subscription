...
async function load(env, key) {
  var raw = await env.REYHOON_KV.get(key);
  if (!raw) return [];
  try { return JSON.parse(raw); } catch (e) { return []; }
}

async function save(env, key, data) {
  await env.REYHOON_KV.put(key, JSON.stringify(data));
}
  ...
  async fetch(request, env) {
    ...
    try {
      if (path === "/api/health" && request.method === "GET") {
        return json({ ok: true, service: "reyhoon" });
      }

      if (path === "/api/menu" && request.method === "GET") {
        var menu = await load(env, "menu");
        return json(menu.filter(function (x) { return x.name !== "__ping__"; }));
      }
      ...
      if (path.indexOf("/api/customers/code/") === 0 && request.method === "GET") {
        ...
        var found = customers.find(function (c) {
          return (c.subscriptionCode || "") === code;
        });
      ...
      if (path === "/api/customers/register" && request.method === "POST") {
        ...
        while (customers.some(function (c) { return c.subscriptionCode === newCode; })) {
      ...
      if (path === "/api/customers" && request.method === "POST") {
        ...
        code = rawCode || String(Math.floor(100000 + Math.random() * 900000));
      ...
      if (path === "/api/payments" && request.method === "POST") {
        ...
        await save(env, "orders", orders);
        await save(env, "customers", customers);
        payments = await load(env, "payments");
        payments.unshift({
