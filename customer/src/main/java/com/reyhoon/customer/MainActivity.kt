...
fun NewRegister(onRegistered: (Customer) -> Unit, onBack: () -> Unit) {
    ...
    val locPermission = rememberLauncherForActivityResult(
        ...
        if (ok) {
            ...
            if (pair != null) {
                lat = pair.first
                lng = pair.second
                locMsg = "موقعیت GPS ثبت شد ✓"
            } else {
                locMsg = "موقعیت یافت نشد — GPS را روشن کنید یا از نقشه انتخاب کنید"
            }
        } else {
    ...
    Text("می‌توانید با GPS یا با انتخاب دستی روی نقشه ثبت کنید.", style = MaterialTheme.typography.bodySmall)
    Button(
        onClick = {
    ...
    ) { Text("📍 ثبت خودکار (GPS)", fontWeight = FontWeight.Bold) }
    OutlinedButton(
        onClick = { showMapPicker = true },
        modifier = Modifier.fillMaxWidth()
    ) { Text("🗺️ انتخاب دستی روی نقشه", fontWeight = FontWeight.Bold) }
    ...
    Button(
        onClick = {
            ...
            val ln = lng
            if (la == null || ln == null) {
                error = "ابتدا لوکیشن را ثبت کنید (GPS یا نقشه)"
                return@Button
            }
            scope.launch {
                loading = true
    ...
    if (showMapPicker) {
        MapLocationPickerDialog(
            initialLat = lat ?: 37.2808,
            initialLng = lng ?: 49.5832,
            onDismiss = { showMapPicker = false },
            onConfirm = { la, ln ->
                lat = la
                lng = ln
                locMsg = "موقعیت از روی نقشه ثبت شد ✓"
                showMapPicker = false
            }
        )
    }
...
fun MapLocationPickerDialog(
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text("انتخاب موقعیت روی نقشه", fontWeight = FontWeight.Bold) },
        text = {
            Column {
                Text(
                    "روی نقشه بزنید یا پین را بکشید، سپس تأیید کنید.",
                ...
                AndroidView(
                    factory = { ctx ->
                        WebView(ctx).apply {
                            ...
                            loadDataWithBaseURL(
                                "https://unpkg.com",
                                mapPickerHtml(initialLat, initialLng),
                                "text/html",
                                "UTF-8",
                                null
                            )
                ...
                Text(
                    "مختصات: " + String.format("%.5f", pickedLat) + " , " + String.format("%.5f", pickedLng),
        ...
        confirmButton = {
            Button(onClick = { onConfirm(pickedLat, pickedLng) }) {
                Text("تأیید این موقعیت", fontWeight = FontWeight.Bold)
            }
        },
        dismissButton = {
...
private class MapJsBridge(private val onPickCb: (Double, Double) -> Unit) {
    @JavascriptInterface
    fun onPick(la: Double, ln: Double) {
...
private fun mapPickerHtml(lat: Double, lng: Double): String {
    val la = lat.toString()
    val ln = lng.toString()
    return (
        ...
        "var map = L.map('map').setView([" + la + ", " + ln + "], 16);" +
        "L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {maxZoom: 19}).addTo(map);" +
        "var marker = L.marker([" + la + ", " + ln + "], {draggable:true}).addTo(map);" +
        "function send(a, b) { try { Android.onPick(a, b); } catch(e) {} }" +
